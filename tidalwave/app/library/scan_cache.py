...
_scan_cache: dict[str, dict | None] = {"recent": None, "collection": None, "stats": None, "catalog": None}
_scan_mtime: dict[str, float] = {"recent": 0.0, "collection": 0.0, "stats": 0.0, "catalog": 0.0}
...
def get_cached(kind: str) -> dict | None:
    ...
    with _scan_lock:
        ...
        if stale and not already_running:
            # Claim the kind atomically under the lock to prevent duplicate scans.
                ...
                for entry in it:
                    ...
                    if len(heap) < _RECENT_LIMIT:
                        ...
                        heapq.heapreplace(heap,
                                           (stat.st_mtime, rel, stat.st_size))
                if scanned > _MAX_SCAN_ENTRIES:
                    break
