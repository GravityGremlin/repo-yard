...
def _init_db() -> None:
    ...
    _migrate_add_column(db, "jobs", "provider", "TEXT DEFAULT ''")
    _migrate_add_column(db, "jobs", "data", "TEXT DEFAULT '{}'")
    db.commit()
    db.close()
...
class Job:
    ...
    provider: str = ""  # which audio provider completed the download
    data: dict = field(default_factory=dict)  # extra metadata (spotify_id, album, isrc, etc.)
...
def list_audit(limit: int = 100, offset: int = 0, job_id: str | None = None) -> list[dict]:
    """Return recent audit log entries, newest first.

    Args:
        limit: Max rows to return.
        offset: Skip this many rows (for pagination).
        job_id: If set, filter to entries matching this job.
    """
    with _lock:
        db = _connect()
        if job_id:
            rows = db.execute(
                "SELECT id, event, job_id, data, created_at FROM audit_log "
