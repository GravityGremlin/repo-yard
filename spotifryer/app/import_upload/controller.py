...
from app.download.beets_integration import organize_with_beets, promote_to_library, trigger_navidrome_scan
...
class _Cancelled(Exception):
    """Raised inside worker when a cancel is detected."""
...
def subscribe_import_job(job_id: str) -> queue.Queue:
...
def unsubscribe_import_job(job_id: str, q: queue.Queue) -> None:
...
def _run_import(job_id: str) -> None:
    ...
    # ── Phase 1: Extract / collect audio files ────────────────────
    try:
        if cancel_event is not None and cancel_event.is_set():
    ...
    # ── Phase 2: Import each file into library ────────────────────
    ...
    for idx, audio_path in enumerate(audio_files):
        ...
        # Try beets first, fall back to promote_to_library
        result = organize_with_beets(audio_path, LIBRARY_DIR)
        if result is None:
            try:
                result = promote_to_library(audio_path, LIBRARY_DIR)
                result = str(result)
            except Exception as exc:
                errors.append(f"{audio_path.name}: {exc}")
                _emit(job_id, {"type": "log", "line": f"Error importing {audio_path.name}: {exc}"})
                continue

        imported_files.append(str(result) if result else str(audio_path))
        _emit(job_id, {"type": "log", "line": f"Imported {audio_path.name}"})
    ...
    if not imported_files:
        job.status = JobStatus.ERROR
        job.error = "; ".join(errors) if errors else "No files imported"
        save_job(job)
        _emit(job_id, {"type": "status", "status": "error"})
        _emit(job_id, {"type": "error", "error": job.error})
        append_audit("import.error", job_id, {"error": job.error})
    ...
    job.files = imported_files
    job.progress = 1.0
    save_job(job)

    _emit(job_id, {"type": "result", "ok": True, "files": imported_files})
    ...
    append_audit("import.completed", job_id, {"files": len(imported_files)})
