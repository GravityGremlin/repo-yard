// spotifryer — minimal client-side JS
...
document.body.addEventListener("htmx:afterRequest", (evt) => {
  if (evt.detail.xhr.status === 503) {
    console.warn("Spotify not connected");
  }
});
...
function downloadItem(url, title, artist, type, ev) {
  ...
  formData.append("kind", type || "track");
...
function subscribeJob(jobId, rowEl) {
  const evtSource = new EventSource(`/download/${jobId}/progress`);
