...
_IS_CONTAINER = os.path.exists("/app/config/qoochie.yaml")
_proj = Path(__file__).resolve().parent.parent

CONFIG_PATH = os.environ.get("CONFIG_PATH",
    "/app/config/qoochie.yaml" if _IS_CONTAINER else str(_proj / "qoochie.yaml"))

# Qobuz credentials supplied via environment (common in container deployments).
QOBUZ_TOKEN = (os.environ.get("QOBUZ_TOKEN") or "").strip()
QOBUZ_USER_ID = int((os.environ.get("QOBUZ_USER_ID") or "0").strip() or "0")
QOBUZ_APP_ID = int((os.environ.get("QOBUZ_APP_ID") or "0").strip() or "0")
QOBUZ_APP_SECRET = (os.environ.get("QOBUZ_APP_SECRET") or "").strip()
...
# regardless of what qoochie.yaml says (it has container paths).
...
if _local:
    ...
    DOWNLOAD_DIR = _get_path("paths.download_dir", "DOWNLOAD_DIR", "/opt/data/qoochie")
    ...
    PLAYLIST_DIR = _get_path("paths.playlist_dir", "PLAYLIST_DIR",
        "/opt/data/qoochie/playlists")

QOBUZ_CONFIG_DIR = _get_path("paths.qobuz_config_dir", "QOBUZ_CONFIG_DIR",
    "~/.config/qobuz")
...
# ── Qobuz ───────────────────────────────────────────────────────
QOBUZ_QUALITY = str(_cfg("qobuz.quality", "HIGH", "QOBUZ_QUALITY"))
MAX_CONCURRENT = int(_cfg("qobuz.max_concurrent", 8, "MAX_CONCURRENT"))

# Quality name → Qobuz format_id.  Iterate the list (highest first) for fallback.
QUALITY_FORMAT_MAP: dict[str, list[int]] = {
    "LOW":      [5],
    "HIGH":     [6, 5],
    "LOSSLESS": [7, 6, 5],
    "HIRES":    [27, 7, 6, 5],
    # Aliases from the YAML config
    "FLAC":     [7, 6, 5],
    "MP3":      [5],
}

def get_format_ids(quality: str | None = None) -> list[int]:
    """Return ordered format_ids for the given quality level, highest first."""
    q = (quality or QOBUZ_QUALITY).upper()
    return QUALITY_FORMAT_MAP.get(q, QUALITY_FORMAT_MAP["HIGH"])
...
IMPORT_STAGING_DIR = _get_path(
    ...
    "/opt/data/qoochie/import_staging" if _IS_CONTAINER else str(_proj / "import_staging"),
)
...
IMPORT_ALLOWED_EXTS: list[str] = _import_exts_raw if isinstance(_import_exts_raw, list) else [
    s.strip() for s in
    str(_import_exts_raw).split(",") if s.strip()
]

_IMPORT_DEFAULT_ARCHIVES = [".zip", ".tar", ".tar.gz"]
_import_archives_raw = _cfg("import_upload.accepted_archives", _IMPORT_DEFAULT_ARCHIVES, "IMPORT_ARCHIVE_EXTS")
IMPORT_ARCHIVE_EXTS: list[str] = _import_archives_raw if isinstance(_import_archives_raw, list) else [
    s.strip() for s in
    str(_import_archives_raw).split(",") if s.strip()
]
