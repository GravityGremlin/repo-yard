...
_IS_CONTAINER = os.path.exists("/app/config/deeznutz.yaml")
_proj = Path(__file__).resolve().parent.parent

CONFIG_PATH = os.environ.get("CONFIG_PATH",
    "/app/config/deeznutz.yaml" if _IS_CONTAINER else str(_proj / "deeznutz.yaml"))

# Deezer ARL cookie supplied via environment (common in container deployments).
# When set, the app will attempt to bootstrap a session from it on startup.
DEEZER_ARL = (os.environ.get("DEEZER_ARL") or "").strip()
...
# regardless of what deeznutz.yaml says (it has container paths).
...
if _local:
    ...
    DOWNLOAD_DIR = _get_path("paths.download_dir", "DOWNLOAD_DIR", "/opt/data/deeznutz")
    ...
    PLAYLIST_DIR = _get_path("paths.playlist_dir", "PLAYLIST_DIR",
        "/opt/data/deeznutz/playlists")

DEEZER_CONFIG_DIR = _get_path("paths.deezer_config_dir", "DEEZER_CONFIG_DIR",
    "~/.config/deeznutz_dz")
...
# ── Deezer ───────────────────────────────────────────────────────
DEEZER_QUALITY = str(_cfg("deezer.quality", "FLAC", "DEEZER_QUALITY"))
MAX_CONCURRENT = int(_cfg("deezer.max_concurrent", 8, "MAX_CONCURRENT"))
...
IMPORT_STAGING_DIR = _get_path(
    ...
    "/opt/data/deeznutz/import_staging" if _IS_CONTAINER else str(_proj / "import_staging"),
)
