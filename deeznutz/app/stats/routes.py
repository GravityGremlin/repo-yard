...
# Config keys the dashboard reads/writes. Names map to keys in deeznutz.yaml
# and the env-var overrides (see app.config).
_CONFIG_KEYS = ("deezer.quality", "deezer.max_concurrent",
                "downloads.organize_with_beets", "downloads.override_existing")

# Quality options exposed by the UI. Mirrors deezer's quality enum.
QUALITY_OPTIONS = ("FLAC", "MP3_320", "MP3_128", "MP4_RA1")
...
# ever mutates deeznutz.yaml; module-level config constants are not re-read,
...
def api_config():
    """Update mutable config keys in deeznutz.yaml.
    ...
    update_map = {
        "quality": ("deezer.quality", str),
        "max_concurrent": ("deezer.max_concurrent", int),
    ...
    for key, value in data.items():
        ...
        try:
            ...
            if yaml_path == "deezer.quality" and value not in QUALITY_OPTIONS:
                error = f"Invalid quality: {value}"
                break
            if yaml_path == "deezer.max_concurrent":
                mc = int(value)
                if mc < 1 or mc > 32:
...
def _apply_config_updates(updates: dict[str, object]) -> None:
    ...
    # constants (DEEZER_QUALITY etc.) are intentionally NOT rebound here — see
