...
# Bind-mounted download dirs (NFS/overlay) can lag: a file deeznutz just wrote
...
def _album_in_library(album_dir: Path) -> str | None:
    ...
    try:
        proc = subprocess.run(
            ["beet", "-c", str(BEETS_DIR / "config.yaml"), "ls", "-f", "$album", query],
...
def _audio_files(d: Path) -> list[Path]:
    return [
        ...
        if p.suffix.lower() in {".flac", ".m4a", ".mp3", ".opus", ".ogg", ".wav"}
    ]
