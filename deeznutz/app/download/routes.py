...
def enqueue():
    ...
    session = get_session()
    if not session:
        return jsonify({"error": "Not connected to Deezer"}), 503
...
def enqueue_discography():
    ...
    session = get_session()
    if not session:
        return jsonify({"error": "Deezer not connected"}), 503
    ...
    for album in albums:
        job = Job(
            url=f"https://deezer.com/album/{album.album_id}",
            title=album.title,
...
def retry_all_errored():
    """Retry every job with status ERROR or CANCELLED."""
    jobs = list_jobs(limit=1000)
    retried = 0
    for job in jobs:
